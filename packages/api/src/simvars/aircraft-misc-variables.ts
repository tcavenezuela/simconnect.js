...
// data types
import { Int32, SInt32, Float64, SFloat64, String128 } from './simvar-utils';
...
const AIRCRAFT_STATES = {
  ...
  'ON ANY RUNWAY': define(`Whether or not the plane is currently on a runway`, Bool, Int32),
  'PLANE IN PARKING STATE': define(
    `Whether or not the plane is currently parked (true) or not (false)`,
    Bool,
    Int32
  ),
  'SURFACE CONDITION': define(`The state of the surface directly under the aircraft`, Enum, Int32),
...
const AIRCRAFT_POSITION_DIRECTION_AND_SPEED = {
  ...
  'PLANE ALT ABOVE GROUND': define(`Altitude above the surface`, Feet, SFloat64),
  'PLANE ALT ABOVE GROUND MINUS CG': define(`Altitude above the surface minus CG`, Feet, SFloat64),
  ...
  'RELATIVE WIND VELOCITY BODY X': define(`Lateral (X axis) speed relative to wind`, Feet, Float64),
  ...
  'VELOCITY BODY X': define(`True lateral speed, relative to aircraft X axis`, Feet, SFloat64),
  'VELOCITY BODY Y': define(`True vertical speed, relative to aircraft Y axis`, Feet, SFloat64),
  'VELOCITY BODY Z': define(`True longitudinal speed, relative to aircraft Z axis`, Feet, SFloat64),
  'VERTICAL SPEED': define(`The current indicated vertical speed for the aircraft`, Feet, SFloat64)
};
...
const AIRSPEED = {
  'AIRCRAFT WIND X': define(`Wind component in aircraft lateral (X) axis`, Knots, Float64),
  'AIRCRAFT WIND Y': define(`Wind component in aircraft vertical (Y) axis.`, Knots, Float64),
  'AIRCRAFT WIND Z': define(`Wind component in aircraft longitudinal (Z) axis.`, Knots, Float64),
  'AIRSPEED BARBER POLE': define(`Redline airspeed (dynamic on some aircraft).`, Knots, Float64),
  ...
  'BARBER POLE MACH': define(`Mach associated with maximum airspeed.`, Mach, Float64),
  ...
  'WINDSHIELD WIND VELOCITY': define(`Longitudinal speed of wind on the windshield.`, Feet, Float64)
};
