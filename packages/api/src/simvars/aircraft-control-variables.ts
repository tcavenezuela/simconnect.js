...
// data units
import { Bool, Number, Degrees, Radians, PercentOver100, Position } from './simvar-utils';
...
const AILERONS = {
  ...
  'AILERON LEFT DEFLECTION PCT': define(`Percent deflection`, PercentOver100, Float64),
  'AILERON POSITION': define(`Percent aileron input left/right`, Position, SFloat64),
  'AILERON RIGHT DEFLECTION': define(`Angle deflection`, Radians, Float64),
  'AILERON RIGHT DEFLECTION PCT': define(`Percent deflection`, PercentOver100, Float64),
...
const ELEVATOR = {
  ...
  'ELEVATOR DEFLECTION PCT': define(`Percent deflection`, PercentOver100, Float64),
  'ELEVATOR POSITION': define(`Percent elevator input deflection`, Position, SFloat64),
  ...
  'ELEVATOR TRIM INDICATOR': define(`Percent elevator trim (for indication)`, Position, Float64),
  ...
  'ELEVATOR TRIM POSITION': define(`Elevator trim deflection`, Radians, SFloat64),
...
const FLAPS = {
  'FLAP DAMAGE BY SPEED': define(`True if flaps are damaged by excessive speed`, Bool, Int32),
  'FLAP POSITION SET': define(`Set the position of the flaps control`, Position, SFloat64),
  'FLAP SPEED EXCEEDED': define(`True if safe speed limit for flaps exceeded`, Bool, Int32),
  ...
  'FLAPS HANDLE INDEX:index': define(`Index of current flap position`, Number, SInt32),
  'FLAPS HANDLE PERCENT': define(`Percent flap handle extended`, PercentOver100, Float64),
  'FLAPS NUM HANDLE POSITIONS': define(`Number of available flap positions`, Number, Int32),
...
const FOLDING_WINGS = {
  'FOLDING WING HANDLE POSITION': define(`True if the folding wing handle is engaged`, Bool, Int32),
...
const RUDDER = {
  ...
  'RUDDER DEFLECTION PCT': define(`Percent deflection`, PercentOver100, Float64),
  ...
  'RUDDER POSITION': define(`Percent rudder input deflection`, Position, SFloat64),
  'RUDDER TRIM': define(`Angle deflection`, Radians, Float64),
  'RUDDER TRIM DISABLED': define(`Whether or not the Rudder Trim has been disabled`, Bool, Int32),
...
const SPOILERS = {
  'SPOILERS ARMED': define(`Checks if autospoilers are armed (true) or not (false)`, Bool, Int32),
  'SPOILER AVAILABLE': define(`True if spoiler system available`, Bool, Int32),
  'SPOILERS HANDLE POSITION': define(`Spoiler handle position`, PercentOver100, Float64),
  'SPOILERS LEFT POSITION': define(`Percent left spoiler deflected`, PercentOver100, Float64),
  'SPOILERS RIGHT POSITION': define(`Percent right spoiler deflected`, PercentOver100, Float64)
};
