...
// data types
import { Int32, SInt32, Float64, SFloat64, String32, SString32 } from './simvar-utils';
...
const ADF = {
  'ADF ACTIVE FREQUENCY:index': define(`ADF frequency. Index of 1 or 2.`, ADF_BCD32, Float64),
  ...
  'ADF RADIAL:index': define(`Current direction from NDB station`, Degrees, Float64),
  ...
  'ADF STANDBY AVAILABLE:index': define(`True if ADF Standby is available`, Bool, Int32),
  'ADF STANDBY FREQUENCY:index': define(`ADF standby frequency`, Hz, Float64),
  'ADF VOLUME': define(`Returns the volume of the ADF`, PercentOver100, Float64)
};
...
const ATC = {
  ...
  'ATC CLEARED LANDING': define(`Whether the ATC has cleared the plane for landing.`, Bool, Int32),
  'ATC CLEARED TAKEOFF': define(`Whether the ATC has cleared the plane for takeoff.`, Bool, Int32),
  'ATC CLEARED TAXI': define(`Whether the ATC has cleared the plane for taxi.`, Bool, Int32),
  ...
  'ATC HEAVY': define(`Is this aircraft recognized by ATC as heavy.`, Bool, SInt32),
  'ATC ID': define(
  ...
  'ATC ON PARKING SPOT': define(`Is ATC aircraft on parking spot.`, Bool, Int32),
...
const COM = {
  ...
  'COM RECEIVE ALL': define(`Toggles all COM radios to receive on`, Bool, Int32),
  ...
  'COM STANDBY FREQUENCY:index': define(`Com standby frequency. Index is 1, 2 or 3.`, BCD16, Int32),
  ...
  'COM TRANSMIT:index': define(`Audio panel com transmit state. Index of 1, 2 or 3.`, Bool, Int32),
...
const FLARM = {
  ...
  'FLARM THREAT DISTANCE': define(`The distance to the FLARM threat object.`, Meters, Float64),
  'FLARM THREAT HEADING': define(`The heading to the FLARM threat object.`, Degrees, Float64),
  'FLARM THREAT RELATIVE ALTITUDE': define(
    `The relative altitude of the threat object.`,
    Meters,
    Float64
  ),
  'FLARM THREAT TIME TO COLLISION': define(`The estimated time to a collision.`, Seconds, Float64),
...
const GPS = {
  ...
  'GPS APPROACH APPROACH INDEX': define(`Index of approach for given airport.`, Number, SInt32),
  'GPS APPROACH APPROACH TYPE': define(`Approach type.`, Enum, SInt32),
  'GPS APPROACH IS FINAL': define(`Is approach transition final approach segment.`, Bool, SInt32),
  'GPS APPROACH IS MISSED': define(`Is approach segment missed approach segment.`, Bool, SInt32),
  ...
  'GPS APPROACH SEGMENT TYPE': define(`Segment type within approach.`, Enum, SInt32),
  'GPS APPROACH TIMEZONE DEVIATION': define(`Deviation of local time from GMT.`, Seconds, Float64),
  'GPS APPROACH TRANSITION ID': define(`ID of approach transition.`, NullUnit, SString32),
  'GPS APPROACH TRANSITION INDEX': define(`Index of approach transition.`, Number, SInt32),
  ...
  'GPS APPROACH WP TYPE': define(`Waypoint type within approach mode.`, Enum, SInt32),
  ...
  'GPS COURSE TO STEER': define(`Suggested heading to steer (for autopilot).`, Radians, SFloat64),
  ...
  'GPS ETA': define(`Estimated time of arrival at destination.`, Seconds, Float64),
  'GPS ETE': define(`Estimated time en route to destination.`, Seconds, Float64),
  ...
  'GPS GROUND MAGNETIC TRACK': define(`Current magnetic ground track.`, Radians, SFloat64),
  'GPS GROUND SPEED': define(`Current ground speed.`, MetersPerSecond, SFloat64),
  'GPS GROUND TRUE HEADING': define(`Current true heading.`, Radians, SFloat64),
  'GPS GROUND TRUE TRACK': define(`Current true ground track.`, Radians, SFloat64),
  ...
  'GPS IS ACTIVE WP LOCKED': define(`Is switching to next waypoint locked.`, Bool, Int32),
  ...
  'GPS IS DIRECTTO FLIGHTPLAN': define(`Is Direct To Waypoint mode active.`, Bool, SInt32),
  ...
  'GPS VERTICAL ERROR': define(`Vertical deviation in meters from GlidePath.`, Meters, SFloat64),
  ...
  'GPS WP ETA': define(`Estimated time of arrival at waypoint.`, Seconds, SFloat64),
  'GPS WP ETE': define(`Estimated time en route to waypoint.`, Seconds, SFloat64),
  ...
  'GPS WP PREV LAT': define(`Latitude of previous waypoint.`, Degrees, SFloat64),
  'GPS WP PREV LON': define(`Longitude of previous waypoint.`, Degrees, SFloat64),
  'GPS WP PREV VALID': define(
    `Is previous waypoint valid (i.e. current waypoint is not the first waypoint).`,
    Bool,
    SInt32
  ),
  'GPS WP TRACK ANGLE ERROR': define(`Tracking angle error to waypoint.`, Radians, SFloat64),
  'GPS WP TRUE BEARING': define(`True bearing to waypoint.`, Radians, SFloat64),
  'GPS WP TRUE REQ HDG': define(`Required true heading to waypoint.`, Radians, SFloat64),
  'GPS WP VERTICAL SPEED': define(`Vertical speed to waypoint.`, MetersPerSecond, SFloat64)
};
...
const NAV = {
  'NAV ACTIVE FREQUENCY:index': define(`Nav active frequency. Index is 1 or 2.`, MHz, Float64),
  'NAV AVAILABLE:index': define(`Flag if Nav equipped on aircraft.`, Bool, Int32),
  'NAV BACK COURSE FLAGS:index': define(`Returns the listed bit flags.`, Flags, Int32),
  ...
  'NAV CODES': define(`Returns bit flags with the listed meaning.`, Flags, Int32),
  ...
  //   ...dataType(`LatLonAlt`),
  // },
  'NAV HAS CLOSE DME': define(`Flag if found a close station with a DME.`, Bool, Int32),
  'NAV HAS CLOSE LOCALIZER': define(`Flag if found a close localizer station.`, Bool, Int32),
  'NAV HAS DME': define(`Flag if tuned station has a DME.`, Bool, Int32),
  'NAV HAS GLIDE SLOPE': define(`Flag if tuned station has a glide slope.`, Bool, Int32),
  'NAV HAS LOCALIZER': define(`Flag if tuned station is a localizer.`, Bool, Int32),
  ...
  'NAV MAGVAR': define(`Magnetic variation of tuned Nav station.`, Degrees, Float64),
  ...
  'NAV RELATIVE BEARING TO STATION': define(`Relative bearing to station.`, Degrees, Float64),
  'NAV SIGNAL': define(`Nav signal strength.`, Number, Int32),
  'NAV SOUND:index': define(`Nav audio flag. Index of 1 or 2.`, Bool, Int32),
  'NAV STANDBY FREQUENCY:index': define(`Nav standby frequency. Index is 1 or 2.`, MHz, Float64),
  'NAV TOFROM': define(
...
const TACAN = {
  ...
  'TACAN OBS:index': define(`The Tacan OBS setting, in degrees.`, Degrees, Float64),
...
const MISC = {
  'COPILOT TRANSMITTER TYPE': define(`On which channel the copilot is transmitting.`, Enum, Int32),
  'COPILOT TRANSMITTING': define(`Whether or not the copilot is transmitting`, Bool, Int32),
  'PILOT TRANSMITTER TYPE': define(`On which channel the pilot is transmitting.`, Enum, Int32),
  'PILOT TRANSMITTING': define(`Whether or not the pilot is transmitting.`, Bool, Int32),
  'RADIO HEIGHT': define(`Radar altitude.`, Feet, Float64),
  'TRANSPONDER AVAILABLE': define(`True if a transponder is available.`, Bool, Int32),
