...
const GENERAL = {
  ...
  'ALTERNATE STATIC SOURCE OPEN:index': define(`Alternate static air source.`, Bool, Int32),
  'ANEMOMETER PCT RPM': define(`Anemometer rpm as a percentage.`, PercentOver100, Float64),
  'ANGLE OF ATTACK INDICATOR': define(`AoA indication.`, Radians, Float64),
  'APPLY HEAT TO SYSTEMS': define(`Used when too close to a fire.`, Bool, SInt32),
  'AUDIO PANEL AVAILABLE': define(`True if the audio panel is available.`, Bool, Int32),
  'AUDIO PANEL VOLUME': define(`The Volume of the Audio Panel.`, Percent, Float64),
  ...
  'CABIN NO SMOKING ALERT SWITCH': define(`True if the No Smoking switch is on.`, Bool, Int32),
  'CABIN SEATBELTS ALERT SWITCH': define(`True if the Seatbelts switch is on.`, Bool, Int32),
  'CANOPY OPEN': define(`Percent primary door/exit open.`, PercentOver100, SFloat64),
  'CARB HEAT AVAILABLE': define(`True if carburetor heat available.`, Bool, Int32),
  'DELTA HEADING RATE': define(`Rate of turn of heading indicator.`, RadiansPerSecond, SFloat64),
  ...
  'FIRE BOTTLE DISCHARGED': define(`True if the fire bottle is discharged.`, Bool, Int32),
  'FIRE BOTTLE SWITCH': define(`True if the fire bottle switch is on.`, Bool, Int32),
  ...
  'GPWS WARNING': define(`True if Ground Proximity Warning System installed.`, Bool, Int32),
  'GYRO DRIFT ERROR': define(`Angular error of heading indicator.`, Radians, Float64),
  'HAS STALL PROTECTION': define(
    `Will return whether the aircraft has stall protection (true) or not (false).`,
    Bool,
    Int32
  ),
  'HEADING INDICATOR': define(`Heading indicator (directional gyro) indication.`, Radians, Float64),
  ...
  'INDUCTOR COMPASS HEADING REF': define(`Inductor compass heading.`, Radians, Float64),
  ...
  'INTERCOM SYSTEM ACTIVE': define(`Whether or not the intercom system is active.`, Bool, Int32),
  'IS ALTITUDE FREEZE ON': define(`True if the altitude of the aircraft is frozen.`, Bool, Int32),
  ...
  'PANEL ANTI ICE SWITCH': define(`True if panel anti-ice switch is on.`, Bool, Int32),
  'PITOT ICE PCT': define(`Amount of pitot ice. 100 is fully iced.`, PercentOver100, Float64),
  ...
  'SPEAKER ACTIVE': define(`Whether or not the speaker is active.`, Bool, Int32),
  ...
  'SUCTION PRESSURE': define(`Vacuum system suction pressure.`, InchesOfMecury, SFloat64),
  'TAILHOOK HANDLE': define(`True if the tailhook handle is engaged.`, Bool, Int32),
  'TAILHOOK POSITION': define(`Percent tail hook extended.`, PercentOver100, SFloat64),
  'TOW RELEASE HANDLE': define(
    `Position of tow release handle. 100 is fully deployed.`,
    PercentOver100,
    Float64
  ),
  'TRUE AIRSPEED SELECTED': define(`True if True Airspeed has been selected.`, Bool, SInt32),
  'TURN COORDINATOR BALL': define(`Turn coordinator ball position.`, Position128, Int32),
  'TURN COORDINATOR BALL INV': define(
    `Turn coordinator ball position inverted (upside down).`,
    Position128,
    Int32
  ),
  'TURN INDICATOR RATE': define(`Turn indicator reading.`, RadiansPerSecond, Float64),
  'TURN INDICATOR SWITCH': define(`True if turn indicator switch is on.`, Bool, Int32),
...
const WATER_BALLAST = {
  ...
  'WATER BALLAST TANK NUMBER': define(`The number of water ballast tank available.`, Number, Int32),
...
const LIGHTS = {
  ...
  'LIGHT LANDING': define(`Light switch state for landing light.`, Bool, SInt32),
  ...
  'LIGHT ON STATES': define(`light on using bit mask (see documentation)`, Mask, Int32),
  ...
  'LIGHT RECOGNITION': define(`Light switch state for the recognition light.`, Bool, SInt32),
  ...
  'MANUAL INSTRUMENT LIGHTS': define(`True if instrument lights are set manually.`, Bool, Int32),
  'STROBES AVAILABLE': define(`True if strobe lights are available.`, Bool, Int32)
};
...
const HYDRAULICS = {
  ...
  'HYDRAULIC SYSTEM INTEGRITY': define(`Percent system functional.`, PercentOver100, Float64)
};
...
const PAYLOAD_STATIONS = {
  ...
  'PAYLOAD STATION COUNT': define(`Number of payload stations (1 to 15).`, Number, Int32),
  'PAYLOAD STATION NAME:index': define(`Descriptive name for payload station.`, NullUnit, String32),
  ...
  'PAYLOAD STATION WEIGHT:index': define(`Individual payload station weight.`, Pounds, SFloat64)
};
...
const YOKE = {
  ...
  'YOKE X INDICATOR': define(`Yoke position in horizontal direction.`, Position, Float64),
  ...
  'YOKE Y INDICATOR': define(`Yoke position in vertical direction.`, Position, Float64),
