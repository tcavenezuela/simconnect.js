...
type ConnectionOptions = { host: string; port: number } & { simConnectCfgIndex: number } & {
...
class SimConnectConnection extends EventEmitter {
  ...
  connect(options?: ConnectionOptions): void {
    if (options && 'host' in options && 'port' in options) {
      ...
      autodetectServerAddress(options?.simConnectCfgIndex).then((address: ConnectionParameters) => {
  ...
  mapInputEventToClientEvent(
    ...
    return this._buildAndSend(
      this._beginPacket(0x11)
        ...
        .putInt32(clientEventUpID === undefined ? SimConnectConstants.UNUSED : clientEventUpID)
        .putInt32(upValue || 0)
        .putInt32(maskable ? 1 : 0)
    );
  }
  ...
  aICreateNonATCAircraft(
    ...
    const packet = this._beginPacket(0x29).putString256(containerTitle).putString(tailNumber, 12);
